...
- **오승민** (백엔드 팀장) // https://github.com/HiSeungmin
...
- **남유람** // https://github.com/yr0202
  : DB모델링, SSE 통신 연결, JPA 엔티티 설계&수정, 비동기 작업 Producer 개발, 프로젝트(TTS, VC, Concat) 상태 저장,비동기 작업처리 실패작업 재실행, 스웨거 api
  명세 작성

- **안재홍** // https://github.com/Jayhong0731
  : DB 모델링, 회원 기능 (회원가입, 이메일 중복 체크, 회원 ID 찾기, 비밀번호 찾기), VC 생성 기능 (기존 기능 통합 및 확장: S3 오디오 파일 업/다운로드, 로컬 및 S3 오디오
  결합 처리, 프로젝트 생성 및 상태 관리, 멀티 파일 처리 지원), Swagger API 문서화 기여, 주요 비즈니스 로직 테스트 및 예외 처리 검증

- **이의준** // https://github.com/LeeEuyJoon
  : CI/CD & 배포 등 인프라, 프로젝트 초기 세팅, DB 모델링, 초기 JPA 엔티티 설계, S3 버킷 오디오 업/다운로드, 오디오 파일 스케줄링 관리, Concat - ( 오디오 삭제 &
  프로젝트 삭제 & 상태 저장 ), 워크스페이스 - (최근 프로젝트 내역 조회 & 최근 생성한 오디오 내역 조회 & 프로젝트 검색 및 페이징 & 생성한 오디오 검색 및 페이징), 구글 TTS 보이스 스타일 데이터
  전처리

- **정원우** // https://github.com/Jeongwonwoo1996
  : TTS 생성 기능 (텍스트 입력 기반 음성 합성 로직 개발 및 Google TTS API 연동), CONCAT 생성 기능 (FFMPEG를 활용한 오디오 파일 병합, 간격 삽입 및 품질 최적화),
  회원 기능 (데이터 저장 및 인증/유효성 검사 로직 개발), DB 모델링, 로깅 및 오류 처리 구조 개선, Swagger API 문서화 기여, 주요 비즈니스 로직 테스트 및 예외 처리 검증

- **조소정** // https://github.com/sojungCho
  : 프로젝트 설계 참여, S3 버킷 오디오 업/다운로드, 오디오 삭제, 오디오 파일 스케줄링 관리, 워크스페이스 - (최근 프로젝트 내역 조회 & 최근 생성한 오디오 내역 조회 & 생성한 오디오
  검색 및 페이징) , 테스트, Swagger Api 작성
